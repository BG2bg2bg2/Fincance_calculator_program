            ...
            while True:
                ...
                if choise==0:
                    ...
                    for num,x in enumerate(expens):
                        print(f"{num+1} for: expense name: {x} amount: ${mount[num]} budget: ${budg[num]}")
                        maxi=num+1
                    inp=utill_functions.get_valid_type(int,"what do you want to change:",valid=(0,maxi))
                    if inp==0:
                        continue
                    expense=utill_functions.get_valid_type(str,"what is the new name: ")
                    amount=utill_functions.get_valid_type(int,"what is the new amount: ")
                    budget=utill_functions.get_valid_type(int,"what is the new budget: ")
                    if utill_functions.get_valid_type(str,f"do you want to change {expenses[inp-1]} to {expense}:{amount}:{budget}(y/n): ",valid=["y", "n"])=="y":
                    ...
                    for x in data:
                        if x["username"]==username and x["password"]==password:
                            userrow = x
                    new_val=utill_functions.get_valid_type(int,f"what do you want to change income ({userrow["income"]}) to (0 to return): ")
                    if new_val==0:
                        continue
                    ...
                    for x in data:
                        if x["username"]==username and x["password"]==password:
                            userrow = x
                    new_val=utill_functions.get_valid_type(int,f"what do you want to change savings ({userrow["savings"]}) to (0 to return): ")
                    if new_val==0:
                        continue
                ...
                if choise==0:
                    ...
                    if len(savings_over_time)==0:
                        ...
                        for num,x in enumerate(savings_over_time):
                            print(f"entry {num+1}: ${x}")
                    while True:
                        val=utill_functions.get_valid_type(int,"what is the new entry: ")
                        if utill_functions.get_valid_type(str,f"are you sure you want to add ${val} to your overtime savings(y/n): ",valid=["y","n"])=="y":
                            break
                    ...
                    for x in data:
                        if x["username"]==username and x["password"]==password:
                            ...
                            savings_over_time=utill_functions.csv_file.from_underscore(x["savings_over_time"])
                    if len(savings_over_time)==0:
                        print("you cant remove any entries because there are none")
                        continue
                    else:
                        print("0 to return")
                        for num,x in enumerate(savings_over_time):
                            print(f"{num+1} to remove entry {num+1}: ${x}")
                            maxi=num+1
                        while True:
                            val=utill_functions.get_valid_type(int,"what do you want to remove: ",valid=(0,maxi))
                            if val==0:
                                break
                            to_remove=savings_over_time[val-1]
                            if utill_functions.get_valid_type(str,f"are you sure you want to remove ${val} from your overtime savings(y/n): ",valid=["y","n"])=="y":
